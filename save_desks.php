<?php
require_once __DIR__ . "/save_desks-conf.php";
...
function get_list_url($name) {
    global $desksids_list_url, $assets_list_url;
    if ($name == "desksids") return $desksids_list_url;
    if ($name == "assetsids") return $assets_list_url;
}

function get_list_data($name) {
    global $use_ntlm_auth, $ntlm_user, $ntlm_password, $pre_resuest_url, $pre_resuest_postdata;

    $url = get_list_url($name);
...
function save_to_csv($filename, $data, $csv_titles) {
    ...
    fputcsv($file, $csv_titles, $csv_delimiter);
...
if ($save_desks) {
    $desksids = get_list_data("desksids");
    $csv_data = [];
    $csv_titles = [];
    $i = 0;
    foreach ($desksids as $row) {
        $desk_id = $row[1];
        if (!$desk_id) continue;
        $desk = send_request("Desk", $desk_id);
        $desk = $desk[0];
        if (!$desk) continue;
        $row = [];
        foreach ($desk as $key => $value) {
            if ($i == 0) $csv_titles[] = iconv("utf-8", $csv_charset, $key);
            $row[] = iconv("utf-8", $csv_charset, $value);
        }
        $csv_data[] = $row;
        $i++;
    }

    save_to_csv($csv_desks_filename, $csv_data, $csv_titles);
}



if ($save_assets) {
    $assetsids = get_list_data("assetsids");
    $csv_data = [];
    $csv_titles = [];
    $i = 0;
    foreach ($assetsids as $row) {
        $asset_id = $row[1];
        if (!$asset_id) continue;
        $asset = send_request("Asset", $asset_id);
        $asset = $asset[0];
        if (!$asset) continue;
        $row = [];
    ...
    save_to_csv($csv_assets_filename, $csv_data, $csv_titles);
}
